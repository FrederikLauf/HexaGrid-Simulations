...
class Agent:
    ...
    def _validate_course(self,direction,radius):
        """Return cell in direction in radius-distance or False, if non-existent."""
        
        current_cell=self._grid._tab[self._position[0]][self._position[1]]
        i=0
        
        while current_cell!=False and i<radius:
    ...
    def _circ_scan_periph(self,R):
        """Return array with parameters of cells in distance R. Inexistent cells
        are formally given the value -1. This method uses an auxiliary grid
    ...
    def _radial_scan(self,radius):
        """Return a list of circular scans, corresponding to radii from 1 to radius.
        Return False if at least one cell is non-existent.
        """
